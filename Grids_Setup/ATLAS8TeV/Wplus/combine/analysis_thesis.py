...
def plot_combined(
    ...
    """
    Combined plots:
      - plot_closure=True  -> 4 rows: main | Grid/NNLO | Grid/Data | k-factors
      - plot_closure=False -> 3 rows: main | Grid/Data | k-factors
    """
    ...
    for col, order in enumerate(use_orders):
        ...
        if plot_closure:
            ...
            ax_r1 = fig.add_subplot(gs[1, col], sharex=ax_main)  # will be hidden
            ax_r2 = ax_r1
            ax_r2 = fig.add_subplot(gs[1, col], sharex=ax_main)  # hepdata ratio here
        ...
        # Hide NNLO y-labels only if plot_closure=True
        if plot_closure and order == "NNLO":
